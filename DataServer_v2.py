...
def get_honeypot_data():
    ...
    while True:
        ...
        print str(datetime.datetime.now() - time_last_request) + " Got %d Hits:" % hits['total']
        if len(hits['hits']) != 0:
            time_last_request = datetime.datetime.utcnow()
...
def process_data(hit):
    ...
    if hit["_type"] == "Cowrie":
        ...
        print "no type matched", hit["_type"]
        print hit
        return
